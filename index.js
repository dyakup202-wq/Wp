...
async function sendOne(accountId, targetId, text) {
  ...
  // Typing — basarisiz olsa da devam (asla durma)
  const typingMs = Math.max(0, Math.min(5000, Number(store.settings.typingDelay) || 0));
  if (typingMs > 0) {
    try {
      const chat = await L.client.getChatById(targetId);
      chatName = (chat && chat.name) || targetId;
      if (chat && chat.sendStateTyping) {
        await chat.sendStateTyping();
        await sleep(typingMs);
      }
    } catch (e) {
      log('Typing atlandi (devam): ' + errText(e), 'warn', accountId);
      await sleep(400);
    }
  }

  // Gonder — timeout/hata olursa birkaç kez dene, dongu zaten disarda tekrarlar
  let lastErr = null;
  for (let attempt = 1; attempt <= 5; attempt++) {
    try {
      ...
      lastErr = e;
      const msg = errText(e);
      log('Gonderim deneme ' + attempt + '/5: ' + msg, 'warn', accountId);
      // Protocol timeout ise biraz bekle yeniden dene — ASLA kalici durma
      await sleep(2000 * attempt);
    }
  }
  if (lastErr) throw new Error('Gonderilemedi (5 deneme): ' + errText(lastErr));
