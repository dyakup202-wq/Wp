...
function normalizeTarget(id) {
  ...
  if (!s) return '';
  // zaten jid
  if (s.includes('@c.us') || s.includes('@g.us') || s.includes('@lid') || s.includes('@s.whatsapp.net')) {
    return s;
  }
  if (s.includes('@')) return s;
  s = s.replace(/[\s+\-()]/g, '');
  // grup id uzun sayi
  if (/^\d{15,}$/.test(s)) return s + '@g.us';
...
function puppeteerOpts() {
  ...
  // protocolTimeout: Runtime.callFunctionOn timed out hatasini onler
  const opts = {
    headless: true,
    args,
    timeout: 0,              // navigasyon timeout yok
    protocolTimeout: 0       // Protocol timeout KAPALI (asla dusmesin)
  };
  for (const pth of [
...
function withTimeout(promise, ms, label) {
  if (!ms || ms <= 0) return promise; // timeout yok — bekle sonsuza kadar
  return Promise.race([
    promise,
    new Promise((_, rej) => setTimeout(() => rej(new Error((label || 'islem') + ' timeout ' + ms + 'ms')), ms))
  ]);
}
 ...
 */
/**
 * getChats KULLANILMIYOR.
 * WhatsApp Web ic Store + Contact uzerinden cekilir (daha stabil).
 */
async function fetchChatsViaStore(client) {
  const page = client.pupPage;
  if (!page) throw new Error('Puppeteer sayfa yok (oturum hazir degil)');
  ...
  const raw = await withTimeout(page.evaluate(() => {
    try {
      ...
      if (Chat) {
        let list = [];
        try {
          if (typeof Chat.getModelsArray === 'function') list = Chat.getModelsArray();
          else if (Chat.models) list = Array.isArray(Chat.models) ? Chat.models : Object.values(Chat.models);
          else if (typeof Chat.map === 'function') {
            const tmp = [];
            Chat.map(function (x) { tmp.push(x); });
            list = tmp;
          }
        } catch (_) {}
        for (let i = 0; i < list.length; i++) {
          ...
          try {
            if (!c || !c.id) continue;
            const id = c.id._serialized || (c.id.user + '@' + (c.id.server || 'c.us'));
            if (!id || seen[id]) continue;
            seen[id] = 1;
            const isGroup = !!(c.isGroup || c.groupMetadata || (c.id && c.id.server === 'g.us'));
            const name = c.formattedTitle || c.name || c.verifiedName || c.pushname
              || (c.contact && (c.contact.name || c.contact.pushname || c.contact.verifiedName))
              || (c.id && c.id.user) || id;
            out.push({ id: String(id), name: String(name), isGroup: !!isGroup, unread: c.unreadCount || 0 });
          } catch (_) {}
        }
      }

      const Contact = window.Store && window.Store.Contact;
      if (Contact) {
        let clist = [];
        try {
          if (typeof Contact.getModelsArray === 'function') clist = Contact.getModelsArray();
          else if (Contact.models) clist = Array.isArray(Contact.models) ? Contact.models : Object.values(Contact.models);
        } catch (_) {}
        for (let i = 0; i < clist.length; i++) {
          const ct = clist[i];
          try {
            if (!ct || !ct.id) continue;
            const id = ct.id._serialized || (ct.id.user + '@' + (ct.id.server || 'c.us'));
            if (!id || seen[id]) continue;
            if (ct.isMe) continue;
            const isGroup = !!(ct.isGroup || (ct.id && ct.id.server === 'g.us'));
            seen[id] = 1;
            const name = ct.name || ct.pushname || ct.verifiedName || ct.shortName || (ct.id && ct.id.user) || id;
            out.push({ id: String(id), name: String(name), isGroup: isGroup, unread: 0 });
          } catch (_) {}
        }
      }
      ...
      return { ok: false, error: String(e && e.message ? e.message : e), chats: [] };
    }
  }), 120000, 'Store.evaluate');

  if (!raw || !raw.ok) {
    throw new Error('Store okunamadi: ' + (raw && raw.error ? raw.error : 'bilinmeyen'));
  }
  return raw.chats || [];
}

async function fetchChatsViaContacts(client) {
  if (typeof client.getContacts !== 'function') return [];
  const contacts = await client.getContacts();
  return (contacts || [])
    .filter((c) => c && c.id && !c.isMe)
    .slice(0, 200)
    .map((c) => ({
      id: c.id._serialized || String(c.id),
      name: c.name || c.pushname || c.number || (c.id && c.id.user) || 'Kisi',
...
async function loadChats(accountId, opts = {}) {
  ...
  L.loadingChats = true;
  try {
    await sleep(opts.skipWait ? 400 : 1200);

    let mapped = [];
    let lastErr = null;

    // 1) Store.Chat / Store.Contact — getChats YOK
    try {
      mapped = await fetchChatsViaStore(L.client);
      if (mapped.length) log('Store ile ' + mapped.length + ' sohbet', 'info', accountId);
    } catch (e) {
      lastErr = e;
      log('Store: ' + errText(e), 'warn', accountId);
    }

    // 2) getContacts fallback
    if (!mapped.length) {
      try {
        mapped = await fetchChatsViaContacts(L.client);
        if (mapped.length) log('Contact API ile ' + mapped.length + ' kisi', 'info', accountId);
      } catch (e) {
        lastErr = e;
        log('Contact: ' + errText(e), 'warn', accountId);
      }
    }

    if (!mapped.length) {
      throw lastErr || new Error('Liste bos — 10sn sonra tekrar dene veya QR yenile');
    }
    ...
    log(mapped.length + ' sohbet/grup (grup: ' + mapped.filter((x) => x.isGroup).length + ') [Store]', 'success', accountId);
    emitStatus();
    return mapped;
  } finally {
...
function createClient(accountId) {
  ...
  client.on('qr', async (qr) => {
    ...
    try {
      ...
      log('QR KOD HAZIR — telefondan okut', 'qr', accountId);
    } catch (e) {
...
async function startAccount(name) {
  ...
  store.accounts[id] = { name: name || ('Hesap ' + Object.keys(store.accounts).length + 1), phone: null, status: 'starting' };
  if (name) store.accounts[id].name = name;
  saveData();
  emitStatus();
  log('Hesap olusturuldu, Chromium / QR basliyor...', 'info', id);

  const client = createClient(id);
  // initialize ARKA PLANDA — HTTP istegi QR beklemeden doner
  client.initialize().then(() => {
    log('Client initialize tamam (veya hazir)', 'info', id);
  }).catch((e) => {
    log('Baslatma hatasi: ' + errText(e), 'error', id);
    if (store.accounts[id]) store.accounts[id].status = 'error';
    saveData();
    emitStatus();
    io.emit('qr', { accountId: id, qr: null, error: errText(e) });
  });
...
function buildBody(text) {
  ...
  const { mentions, tags } = parseMentions();
  if (tags.length) {
    // WhatsApp mention: metinde @numara + options.mentions
    msg = tags.join(' ') + (msg ? ' ' + msg : '');
  }
  return {
    body: msg.slice(0, 60000),
    mentions
  };
}

async function sendViaStore(client, chatId, text, mentionIds) {
  const page = client.pupPage;
  if (!page) throw new Error('pupPage yok');
  const result = await page.evaluate(async (cid, content, mentions) => {
    try {
      const Store = window.Store;
      if (!Store || !Store.Chat) return { ok: false, error: 'Store.Chat yok' };

      let chat = null;
      try { if (Store.Chat.get) chat = Store.Chat.get(cid); } catch (_) {}
      if (!chat && Store.Chat.find) {
        try { chat = await Store.Chat.find(cid); } catch (_) {}
      }
      if (!chat) return { ok: false, error: 'chat yok: ' + cid };

      // mention contact objeleri
      let mentionList = [];
      if (mentions && mentions.length && Store.Contact) {
        for (let i = 0; i < mentions.length; i++) {
          const mid = mentions[i];
          let c = null;
          try { c = Store.Contact.get(mid); } catch (_) {}
          if (!c && Store.Contact.find) {
            try { c = await Store.Contact.find(mid); } catch (_) {}
          }
          if (c) mentionList.push(c);
        }
      }

      if (window.WWebJS && typeof window.WWebJS.sendMessage === 'function') {
        await window.WWebJS.sendMessage(cid, content, { mentions: mentionList.length ? mentions : undefined });
        return { ok: true, via: 'WWebJS' };
      }
      if (typeof Store.SendMessage === 'function') {
        await Store.SendMessage(chat, content, {
          linkPreview: null,
      ...
      if (chat.sendMessage) {
        await chat.sendMessage(content, { mentions: mentionList });
        return { ok: true, via: 'chat.sendMessage' };
      }
      return { ok: false, error: 'send API yok' };
    } catch (e) {
      return { ok: false, error: String(e && e.message ? e.message : e) };
    }
  }, chatId, text, mentionIds || []);

  if (!result || !result.ok) throw new Error('Store: ' + (result && result.error ? result.error : 'fail'));
  return result;
}

async function sendOne(accountId, targetId, text) {
  const L = live[accountId];
  if (!L?.ready || !L.client) throw new Error('Hesap hazir degil — once baglan');
  targetId = normalizeTarget(targetId);
  if (!targetId) throw new Error('Hedef bos — grup/kisi sec');

  const built = buildBody(text);
  const body = built.body;
  const mentions = built.mentions || [];
  if (!String(body).trim()) throw new Error('Mesaj bos');

  log(
    'Gonderiliyor → ' + targetId +
    (mentions.length ? ' | etiket:' + mentions.length : '') +
    ' | ' + body.slice(0, 40),
    'info',
    accountId
  );
  ...
  // Typing (hata yut)
  const typingMs = Math.max(0, Math.min(4000, Number(store.settings.typingDelay) || 0));
  if (typingMs > 0) {
    try {
  ...
  const opts = mentions.length ? { mentions } : undefined;
  let lastErr = null;

  // Yontem 1: client.sendMessage + mentions
  try {
    if (opts) await L.client.sendMessage(targetId, body, opts);
    else await L.client.sendMessage(targetId, body);
    log('OK client.sendMessage → ' + body.slice(0, 40), 'send', accountId);
    lastErr = null;
  } catch (e1) {
    lastErr = e1;
    log('client.sendMessage hata: ' + errText(e1), 'warn', accountId);

    // Yontem 2: chat.sendMessage
    try {
      const chat = await L.client.getChatById(targetId);
      if (opts) await chat.sendMessage(body, opts);
      else await chat.sendMessage(body);
      log('OK chat.sendMessage → ' + body.slice(0, 40), 'send', accountId);
      lastErr = null;
    } catch (e2) {
      lastErr = e2;
      log('chat.sendMessage hata: ' + errText(e2), 'warn', accountId);

      // Yontem 3: Store
      try {
        await sendViaStore(L.client, targetId, body, mentions);
        log('OK Store → ' + body.slice(0, 40), 'send', accountId);
        lastErr = null;
      } catch (e3) {
        lastErr = e3;
        log('Store hata: ' + errText(e3), 'warn', accountId);
      }
    }
  }

  if (lastErr) throw new Error('Gonderilemedi: ' + errText(lastErr));
...
function startLoop() {
  if (loopRunning) {
    log('Dongu zaten acik', 'warn');
    return;
  }
  const accId = store.settings.accountId;
  log('Baslat istegi | hesap=' + (accId || 'YOK') + ' hedef=' + (store.settings.targetId || 'YOK') + ' mesaj=' + store.messages.length, 'info');
  if (!accId) return log('Hesap secilmedi', 'error');
  if (!live[accId] || !live[accId].ready) return log('Hesap bagli degil (ready degil)', 'error');
  if (!store.settings.targetId) return log('Hedef yok — listeden sec veya ID yaz', 'error');
  ...
  const tick = async () => {
    ...
    try {
      ...
      const te = errText(e);
      log('Gonderme (devam edecek): ' + te, 'error', accId);
      // Timeout / protocol / gecici hatalarda ASLA durma — sadece bekle ve tekrar dene
      // Kullanici Durdur demedikce dongu sonsuz devam
      const wait = /timeout|protocol|timed out|Evaluation failed|Target closed|Session closed/i.test(te)
        ? Math.max(8000, delay)
        : Math.max(4000, delay);
      if (loopRunning) loopTimer = setTimeout(tick, wait);
    }
  };
  tick();
}
...
app.post('/api/account/add', async (req, res) => {
  try {
    log('API /account/add cagrildi', 'info');
    const r = await startAccount((req.body?.name || '').trim());
    res.json(r);
  } catch (e) {
    log('API add hata: ' + errText(e), 'error');
...
app.get('/api/account/:id/qr', async (req, res) => {
  try {
    const id = req.params.id;
    const L = live[id];
    if (!L) return res.json({ ok: false, error: 'hesap yok' });
    if (L.ready) return res.json({ ok: true, connected: true, qr: null });
    if (!L.qr) return res.json({ ok: true, pending: true, qr: null });
    const qrcode = require('qrcode');
    const dataUrl = await qrcode.toDataURL(L.qr);
    res.json({ ok: true, qr: dataUrl, accountId: id });
  } catch (e) {
    res.status(500).json({ ok: false, error: errText(e) });
  }
});
...
app.post('/api/loop/start', (req, res) => {
  try {
    const b = req.body || {};
    if (b.accountId) store.settings.accountId = String(b.accountId);
    if (b.targetId) store.settings.targetId = normalizeTarget(b.targetId);
    if (b.delay) store.settings.delay = Math.max(2500, Number(b.delay) || 4500);
    if (b.typingDelay !== undefined) store.settings.typingDelay = Math.max(0, Number(b.typingDelay) || 0);
    if (b.prefix !== undefined) store.settings.prefix = String(b.prefix || '');
    if (typeof b.usePrefix === 'boolean') store.settings.usePrefix = b.usePrefix;
    if (Array.isArray(b.messages) && b.messages.length) {
      store.messages = b.messages.map((m) => String(m).trim()).filter(Boolean);
    }
    saveData();
    log('API start | acc=' + store.settings.accountId + ' target=' + store.settings.targetId + ' msgs=' + store.messages.length + ' prefix=' + (store.settings.usePrefix ? store.settings.prefix : 'kapali'), 'info');
    startLoop();
    res.json({
      ok: true,
      running: loopRunning,
      delay: store.settings.delay,
      targetId: store.settings.targetId,
...
socket.on('qr',d=>{
  ...
  if(!box) return;
  if(d.error){
    box.innerHTML='<div style="color:#e74c3c;padding:12px">QR hata: '+esc(d.error)+'</div>';
    return;
  }
  if(d.connected){
    box.innerHTML='<div style="color:#25d366;padding:12px">Baglandi</div>';
    return;
  }
  if(d.qr){
    box.innerHTML='<img src="'+d.qr+'" style="max-width:220px;border:3px solid #25d366;border-radius:8px"><div style="font-size:11px;color:#888;margin-top:6px">Okut: '+esc(d.accountId)+'</div>';
  }
});
...
async function addAcc(){
  const box=document.getElementById('qrBox');
  box.innerHTML='<div style="color:#e3b341;padding:16px">QR hazirlaniyor... Chromium aciliyor (10-40sn)</div>';
  const name=document.getElementById('accName').value.trim();
  try {
    const res=await api('/api/account/add','POST',{name:name});
    if(!res || res.ok===false){
      box.innerHTML='<div style="color:#e74c3c">Hata: '+esc((res&&res.error)||'bilinmeyen')+'</div>';
      alert('Hata: '+(res&&res.error||'QR baslatilamadi'));
      return;
    }
    document.getElementById('accName').value='';
    const accId=res.accountId;
    box.innerHTML='<div style="color:#e3b341;padding:16px">QR bekleniyor...<br><small>'+esc(accId||'')+'</small></div>';
    load();
    // Socket gelmezse API ile cek (30 deneme)
    if(accId){
      let tries=0;
      const poll=async()=>{
        tries++;
        if(tries>40){ box.innerHTML='<div style="color:#e74c3c">QR zaman asimina ugradi. Tekrar +QR dene.</div>'; return; }
        const q=await api('/api/account/'+accId+'/qr');
        if(q&&q.connected){ box.innerHTML='<div style="color:#25d366">Baglandi</div>'; load(); return; }
        if(q&&q.qr){
          box.innerHTML='<img src="'+q.qr+'" style="max-width:220px;border:3px solid #25d366;border-radius:8px"><div style="font-size:11px;color:#888;margin-top:6px">Okut: '+esc(accId)+'</div>';
          return;
        }
    ...
    box.innerHTML='<div style="color:#e74c3c">Istek hatasi: '+esc(e.message)+'</div>';
  }
}
...
async function startL(){
  ...
  const body={
    accountId:document.getElementById('accSel').value,
    targetId:document.getElementById('target').value.trim(),
    delay:Math.max(2500,parseInt(document.getElementById('delay').value)||4500),
    typingDelay:parseInt(document.getElementById('typing').value)||0,
    prefix:document.getElementById('prefix').value.trim(),
    usePrefix:document.getElementById('usePrefix').checked,
    messages:messages
  };
  if(!body.accountId)return alert('Hesap sec');
  if(!body.targetId)return alert('Hedef sec veya yaz');
  if(!messages.length)return alert('Mesaj yaz');
  await api('/api/messages','POST',{messages});
  await api('/api/settings','POST',body);
  const res=await api('/api/loop/start','POST',body);
  if(!res.ok)alert(res.error||'Baslamadi');
  else if(!res.running)alert('Dongu baslamadi — loga bak (hesap/hedef)');
}
